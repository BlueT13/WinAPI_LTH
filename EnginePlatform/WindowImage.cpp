...
// 헤더랑 다르게 라이브러리는 #pragma comment 통해서 추가해야 한다.
...
// GDIPlus를 사용해야 한다.
// GIDPlus는 윈도우가 초기 윈도우의 그래픽 시스템을 개선해서 추가한 라이브러리
...
bool UWindowImage::Load(UWindowImage* _Image)
{
	...
	else if (".PNG" == UpperExt)
	{
		...
		// Gdiplus헤더의 함수들은 전부 다 Gdiplus 네임스페이스 안에 들어있다.
		// GDi를 사용하겠다는 시작 
		Gdiplus::GdiplusStartupInput gdistartupinput;
		Gdiplus::GdiplusStartup(&gdiplusToken, &gdistartupinput, nullptr);

		// 유니코드로 되어있음
		// 그래서 멀티바이트 경로를 유니코드 경로로 변경하는 함수가 필요
		std::wstring wPath = UEngineString::AnsiToUniCode(Path.GetFullPath());

		// 어떤 문자열에 기반해서 만들어졌는지 알 수 없으므로
		// 모든 문자열에 대비할 방법을 찾아야 한다.
	...
	// 그릴수있고 이미지에 간섭할수 있는 권한
	// 어디에 그릴거냐.
	// 어디에 속해서 어디에 그려질 DC인지를 넣어줘야 한다.
	// 당연히 하나뿐인 mainwindow의 DC를 넣어주면 된다.
	// 그릴 수 있는 권한이 자신이 뭘 그려야하는지를 알려줘야 한다.
	// ImageDC에게 BitMap을 그리라고 명령
...
void UWindowImage::BitCopy(UWindowImage* _CopyImage, const FTransform& _Trans)
{
	...
	BitBlt(
		...
		0,								// int x1,  
		0,								// int y1, 
		SRCCOPY							// DWORD rop => 이미지 그대로 고속 복사
	);
}
...
void UWindowImage::TransCopy(UWindowImage* _CopyImage, const FTransform& _Trans, int _Index, Color8Bit _Color)
{
	...
	TransparentBlt(
		...
		ImageScaleX, 					// int y1, 
		ImageScaleY, 					// int y1, 
		_Color.Color					// DWORD rop => 이미지 그대로 고속 복사
	);
	// 이미지의 어떤 부위를 그릴지 정해줄 수 있다.
}
